...
def get_schedule(halls_show_count, movie_hall_seq, pref_starts=None):
    ...
    Использует динамическое программирование (DP) для нахождения точного решения.
    Для каждого зала задача решается независимо, поскольку залы не взаимодействуют.
    ...
    Обратный проход (i = n-1 .. 0):
      ...
      suf[i][t] = max(dp[i][t'] для t' >= t) — суффиксный максимум, позволяющий
                 быстро находить оптимальное продолжение из любого момента времени.

    Переход:
      dp[i][t] = sales[movie_i][t] + suf[i+1][t + len_i + 1]
                                           ^^^^^^^^^^^^^^^^^^
                      следующий сеанс может начаться не раньше t + len + 1
                      (длина фильма + 5-минутный перерыв)

    Прямой проход:
      Для каждого сеанса i выбираем t, максимизирующий dp[i][t],
      при условии t >= earliest (время окончания предыдущего сеанса + перерыв).
      Это гарантирует физическую реализуемость расписания.

    Оптимизации реализации:
    ----------------------
    1. Offset-массивы: dp_arrays и suf_arrays используют смещение по lb[i]
       для экономии памяти и ускорения доступа
    2. Предварительные вычисления: lb и ub массивы определяют допустимые
       границы времени для каждого сеанса, учитывая минимальные требования
       к размещению всех сеансов
    3. Раннее прерывание: если суммарная минимальная длительность всех сеансов
       превышает доступное время T, задача объявляется неразрешимой

    Сложность: O(n * T) на зал, где n — число сеансов, T = 288 (количество
    временных слотов в день). Это значительно быстрее CP-SAT с add_element,
    при этом даёт точное оптимальное решение.

    Args:
        halls_show_count: {зал: число_сеансов} — количество сеансов в каждом зале
        movie_hall_seq: {(зал, индекс): фильм} — последовательность фильмов в залах
        pref_starts: {(зал, индекс): время} — предпочтительные времена начала (не используется)
                     оставлено для совместимости сигнатур с другими методами генерации

    Returns:
        (schedule, solve_time):
            schedule — {(зал, индекс): {movie, start, end}} или None, если решения нет
            solve_time — время выполнения алгоритма в секундах
    """
