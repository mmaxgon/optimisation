...
> 💡 `pref_starts` позволяет частично сохранить хорошее локальное расположение сеансов, даже если порядок фильмов меняется.
...
> 🔍 Каждая попытка включает полный проход через Level 3 (DP), чтобы точно оценить потенциал последовательности. Это обеспечивает корректное сравнение между "наследуемым" и "случайным" порядком.
...
> 💡 `NEG_INF = -10**9` используется для инициализации недопустимых состояний в DP.
...
| `SEED` | 42 | Seed для воспроизводимости (устанавливается в `main()`) |
| `n_candidates` | 3 | Число кандидатов в `get_best_sequence` |

> 🔐 `random.seed(SEED)` и `np.random.seed(SEED)` гарантируют воспроизводимость при фиксированном числе процессов.
...
$$\tilde{n}_{h,m} = n_{h,m} + \varepsilon, \qquad \varepsilon \sim \mathcal{N}(0,\; \alpha^k \cdot \sigma_y),\quad \sigma_y = 3.0$$

> ⚠️ Масштаб шума **не зависит** от верхней границы $ \bar{n}_{h,m} $. Это предотвращает предвзятость к фильмам с большим лимитом.
...
Потомок подаётся в `search_neighborhood` с уменьшенным шумом (`sigma_y * 0.7`), чтобы не разрушать унаследованные свойства. Количество потомков: `n_crossover = N_keep // 3`, округлённое вниз.
...
Каждые 10 итераций (начиная с итерации 0) генерируется одно полностью случайное решение через `sample_initial()`. Это обеспечивает регулярное внесение разнообразия, не перегружая популяцию.

> 🔄 Иммиграция помогает избежать преждевременной сходимости.
...
- Для эффективной загрузки задачи группируются в батчи.
- При инициализации запускается `N` задач сразу. На каждой итерации запускается до `N_keep + n_crossover + (1 if immigration)` задач.
...
```python
solution = (
    movie_hall_count,   # dict {(hall, movie): count} — результат Level 1 (SCIP)
    schedule,           # dict {(hall, idx): {"movie": str, "start": int, "end": int}} — полное расписание (результат get_schedule)
    f_val               # float — суммарные продажи (get_goal(schedule))
)
```
...
| Метаэвристика (`schedule-global.py`) | 1202 | 20 с | 2,8% |
...
- Warm start предоставляет CP-SAT качественное начальное решение (f = 1202), от которого решатель может двигаться к лучшим значениям.
- Результат: **f = 1257** — на 20 единиц (1,6%) лучше, чем чистый CP-SAT (f = 1237) за сравнимое время (636 с vs 615 с).

**Типичный вывод:**

```text
МЕТАЭВРИСТИКА: f = 1202, время = 20.0s
...
Итого: f = 1202, время = 20.2s, вычислено 2897 точек
```

> ✅ Обновлён пример вывода: количество вычисленных точек соответствует реальному поведению (`total_evals` из кода).
